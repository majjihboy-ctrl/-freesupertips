  ...
  return (
    <section className="py-20 px-4 sm:px-6 lg:px-8 bg-bg-base">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-3xl sm:text-4xl font-bold text-center text-white mb-12">Latest Sports News</h2>

        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {blogPosts.map((post) => (
            <article key={post.id} className="bg-bg-surface rounded-2xl p-8 shadow-lg hover:shadow-xl transition-all border border-bg-surface-hover flex flex-col">
              <div className="text-xs font-bold text-brand-green uppercase tracking-wider mb-3">{post.category}</div>
              <h3 className="text-xl font-bold text-white mb-4 leading-tight flex-1">
                <Link to={`/blog/${post.id}`} className="hover:text-brand-green transition-colors">{post.title}</Link>
              </h3>
              <p className="text-slate-400 text-sm mb-6 leading-relaxed">{post.excerpt}</p>
              <Link to={`/blog/${post.id}`} className="text-brand-green font-bold text-sm hover:underline">
                Read More →
              </Link>
            </article>
          ))}
        ...
        <div className="text-center mt-12">
          <Link to="/blog" className="inline-block px-8 py-3.5 bg-brand-green text-white rounded-full font-bold hover:bg-brand-green-hover transition-all shadow-lg hover:-translate-y-0.5">
            View All Articles
          </Link>
        </div>
      </div>
